...
def evaluate_pairs(res_root,gt_root):
    img_names = os.listdir(res_root)
    gt_names = os.listdir(gt_root)
    score_dict = OrderedDict()
    for index in range(len(img_names)):
        print('-'*10,img_names[index],gt_names[index])
        im_path = os.path.join(res_root, img_names[index])
        gt_path = os.path.join(gt_root, gt_names[index])
        pred = cv2.imread(im_path)
        gt = cv2.imread(gt_path)
        score_dict[img_names[index]] = get_binary_classification_metrics(pred,
                                                                 gt,
                                                                 125)
...
if __name__ == '__main__':
    res_root='./Results_Shadow_Detection/UCF/'
    gt_root ='./Results_Shadow_Detection/UCF_gt'
